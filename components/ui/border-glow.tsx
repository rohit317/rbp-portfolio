...
function buildMeshGradients(colors: string[]): string[] {
  ...
  for (let i = 0; i < 7; i++) {
    ...
    const index = Math.min(colorIndex, palette.length - 1);
    const c = palette[index] ?? palette[0];
    gradients.push(`radial-gradient(at ${GRADIENT_POSITIONS[i]}, ${c} 0px, transparent 50%)`);
  }
  gradients.push(`linear-gradient(${palette[0]} 0 100%)`);
  return gradients;
}
...
const BorderGlow: React.FC<BorderGlowProps> = ({
  ...
  return (
    <div
      ...
      className={`relative grid isolate border border-foreground/5 dark:border-foreground/10 ${className}`}
      style={{
